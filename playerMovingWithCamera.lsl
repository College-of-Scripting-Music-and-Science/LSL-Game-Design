...
CameraLookingDown()
{
    ...
    llSetCameraEyeOffset(<0, 0, 5.0> * sitRot);
    llSetCameraAtOffset(<0, 0, 1> * sitRot);
}
...
default
{
    ...
    control(key id, integer level, integer edge)
    {
        ...
        {
            llSetLinkPrimitiveParamsFast(LINK_THIS,[PRIM_POSITION, llGetLocalPos() + <0,0,speed> ]);
       }
       if (button & CONTROL_DOWN)          
       {
            llSetLinkPrimitiveParamsFast(LINK_THIS,[PRIM_POSITION, llGetLocalPos() + <0,0,-speed> ]);
       }
    }
